...
export function AddPosicaoModal({ open, onClose }: Props) {
  ...
  function handleSubmit(e: React.FormEvent) {
    ...
    const atual = parseFloat(form.preco_atual_cache.replace(',', '.'));

    if (!form.ativo_id || !form.corretora_id || isNaN(qtd) || isNaN(medio) || isNaN(atual)) return;
  ...
  const selectedAtivo = carteira?.ativos.find((a) => a.id === form.ativo_id);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Adicionar / Atualizar Posição">
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1.5">
          ...
          {carteira.ativos.length === 0 && (
            <p className="text-xs text-amber-500">
              Nenhum ativo cadastrado. Adicione um ativo primeiro.
            </p>
          )}
        </div>

        <div className="space-y-1.5">
          <label className="text-xs font-medium text-slate-400">Corretora</label>
          <select
        ...
        <div className="grid grid-cols-3 gap-3">
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              step="any"
              min="0"
              value={form.preco_atual_cache}
              onChange={(e) => setForm((f) => ({ ...f, preco_atual_cache: e.target.value }))}
              placeholder="38.20"
              className="w-full bg-surface-0 border border-slate-600 rounded-lg px-3 py-2 text-sm mono text-slate-100 placeholder-slate-600 focus:outline-none focus:border-sky-500"
            />
          </div>
        ...
        {selectedAtivo?.atualizacao === 'automatica' && (
          <p className="text-xs text-slate-500">
            Para ativos automáticos, o preço atual será atualizado via BRAPI na tabela de posições.
          </p>
        )}
